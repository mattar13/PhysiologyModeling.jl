...
dosetimes = [5e3, 150e3]
affect!(integrator) = integrator.u[12] += 0.100
cb = PresetTimeCallback(dosetimes, affect!)

#Set up the problem
prob = SDEProblem(SAC_ODE, noise1D, u0, tspan, p0)
@time sol = solve(prob, SOSRI(), reltol = 2e-2, abstol = 2e-2, 
...
ax2c = Axis(fig1[2,3], title = "Dopamine (Dt)")
ax3c = Axis(fig1[3,3], title = "cAMP (At)")
ax4c = Axis(fig1[4,3], title = "TREK (Bt)")
...
lines!(ax3a, Time, map(t -> sol(t)[14], Time))
...
lines!(ax2c, Time, map(t -> sol(t)[12], Time))
lines!(ax3c, Time, map(t -> sol(t)[7], Time))
lines!(ax4c, Time, map(t -> sol(t)[8], Time))

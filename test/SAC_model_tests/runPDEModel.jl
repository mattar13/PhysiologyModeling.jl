...
ax22 = Axis(fDIFF[2,2])
idx = 8
surface!(ax11, cell_map.xs, cell_map.ys, sol(0.0)[:, idx])
scatter!(ax21, cell_map.xs, cell_map.ys, color = sol(0.0)[:, idx], markersize = 15.0)
...
surf2 = surface!(ax12, cells[:, 1], cells[:, 2], sol(0.0)[:, idx], #=colorrange = (0.0, 1.0),=# markersize = 35.0)
scat2 = scatter!(ax22, cells[:, 1], cells[:, 2], color = sol(0.0)[:, idx], #=colorrange = (0.0, 1.0),=# markersize = 15.0)

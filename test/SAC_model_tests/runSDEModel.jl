...
#Extract and modify parameters
p0_dict = SAC_p0_dict()
p0 = extract_p0(p0_dict, mode = :GLUTAMATE)
p0_dict["g_GLUT"]
#Extract and modify initial conditions
u0_dict = SAC_u0_dict()
u0 = extract_u0(u0_dict, mode = :GLUTAMATE)
#Set up the problem
prob_func(du, u, p, t) = SAC_ODE_GLUT(du, u, p, t)
prob = SDEProblem(prob_func, noise1D, u0, tspan, p0)
@time sol = solve(prob, SOSRI(), reltol = 2e-2, abstol = 2e-2, progress = true, progress_steps = 1)
...
ax8 = Axis(fSDE[1,3], title = "Glutamate (Gt)")
ax9 = Axis(fSDE[2,3], title = "Gq (Qt)")
